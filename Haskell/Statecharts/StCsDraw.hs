...
consDescOf mmi stc dnm = 
  let (sts, ts) = gDescInfo (stc_sg_cmm mmi) stc dnm in
  let descsOf s = foldr(\dnm' ds->(consDescOf mmi stc dnm'):ds) [] (gDescs stc s) in
  let sts' = foldr (\s sts->(State (nmOfNamed' stc s) (frCMMTyToStTy  (gCMMStTy (stc_sg_cmm mmi) stc s)) $ descsOf s):sts) [] sts in
...
wrStCAsGraphviz mmi stc = wrStCDrawing (stc_sg_cmm mmi) stc $ drawStC mmi stc