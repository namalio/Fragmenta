...
openACOf :: MMInfo String String -> GrwT [Char] [Char] -> String -> Bool
openACOf mmi pc n = 
  let ac = nextAfterC mmi pc n in
  isSomething ac && openAC pc (the ac)

atomBranch::MMInfo String String->PC String String->Id->[Id]->(PCT, Set Id, Set Id)
...
compound::MMInfo String String->PC String String->Id->[Id]->(PCT, [Id], [Id])
...
compoundAB::MMInfo String String->PC String String->Id->[Id]->(PCT, [Id], [Id])
...
consRefOrcompound::MMInfo String String->PC String String->Id->[Id]->(PCT, [Id], [Id])
consRefOrcompound mmi pc n gcs 
    | n `elem` gcs = (Ref n Nothing [] [], [], [])
    | otherwise = compoundAB mmi pc n gcs

refLeaf :: Foldable t => MMInfo String String -> GrwT String String -> String -> t String -> (PCT, [String], [a])
...
refBranch :: MMInfo String String -> GrwT String String -> [Char] -> [[Char]] -> (PCT, Set Id, Set Id)
...
opTree::MMInfo String String->PC String String->Id->[Id]->(PCT, [Id], [Id])
opTree mmi pc n gcs = 
...
consOptBranch::MMInfo String String->PC String String->Maybe Id->[Id]->(PCT, Set Id, Set Id)
consOptBranch mmi pc on gcs
   | isNil on = (NilT, nil, nil) 
   | otherwise = consBranch mmi pc (the on) gcs

consBranch::MMInfo String String->PC String String->Id->[Id]->(PCT, Set Id, Set Id)
...
opBranches::MMInfo String String->PC String String->TOp->[Id]->[Id]->(PCT, [Id], [Id])
...
seqCTs::MMInfo String String->PC String String->[Id]->[Id]->([CT], [Id], [Id])
seqCTs _ _ [] gcs = ([], [], gcs)
